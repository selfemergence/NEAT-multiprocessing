...
class Population:
    ...
    def __init__(self, config):
        ...
        self.fitness_function = self.Config.fitness_fn
...
#                genome.fitness = self.fitness_function(genome)
            ...
            for i in range(len(self.population)):
                p = multiprocessing.Process(target=self.fitness_function, 
                                            args=[i, self.Config.TRAIN_EPISODES, False, q])
                p.start()
                processes.append(p)
            ...
            #fitnesses = [q.get() for p in processes]
            #for i in range(len(self.population)):
                #self.population[i].fitness = q.get()#fitnesses[i]
#                
    ...
    def set_initial_population(self):
        ...
        for i in range(self.Config.POPULATION_SIZE):
            ...
            # create nodes by pool
            '''
            inputs = self.pool.map(
                new_genome.add_node_gene,
                ['input' for _ in range(self.Config.NUM_INPUTS)]
            )
            
            outputs = self.pool.map(
                        new_genome.add_node_gene,
                        ['output' for _ in range(self.Config.NUM_OUTPUTS)]
                    )
            '''
            ...
            # using multiprocessing to create connections
#            with concurrent.futures.ProcessPoolExecutor() as executor:
#               results = [executor.submit(new_genome.add_connection_gene, args=[input.id, output.id]) \
